...
class GlobalStateFactoryMethodsResourceTest extends MediaWikiIntegrationTestCase {
	...
	public function testServicesSorted( array $services ): void {
		...
		usort( $sortedServices, static function ( $serviceA, $serviceB ) {
			$isRepoServiceA = strpos( $serviceA, 'EntitySchema.' ) === 0;
			$isRepoServiceB = strpos( $serviceB, 'EntitySchema.' ) === 0;
		...
		$this->assertSame( $sortedServices, $services,
			'Services should be sorted: first all MediaWiki services, then all EntitySchema ones.' );
	}
